---
title: "MB5370 Module 04. Introduction to Data Science"
author: "Claire Kirby"
...
```{r}
#nrow defines number of rows
#ncol defines number of columns
...
```{r}
#geom_line connects the points in the order in the data, while geom_smooth fits a smooth curve to show overall trend which makes more sense for this plot
```
...
```{r}
# R fits statistical model to data by joining the dotsss in a non-linear matter drawing a smooth curve through them
```
...
```{r}
#The argument se controls where the grey shaded confidence band is shown. Therefore, se = FALSE hides the confidence inerval 
```
...
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point(mapping = aes(color = class)) + 
  geom_line (data = filter(mpg, class == "subcompact"))

ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point(mapping = aes(color = class)) + 
  geom_boxplot(data = filter(mpg,class == "subcompact"))
...
ggplot(data = mpg, mapping = aes(x = displ, fill = class)) +
  ...
  ggplot(data = mpg, mapping = aes(x = displ, y = hwy, fill = class))+
    geom_area(aplha =0.6, position = "stack")
...
```{r}
#no, they will look the same as containing similar dataset and variables. The only difference is the style of coding
```
...
```{r}
#I am not plotting the raw data,so I want frequency not count
```
...
ggplot(mpg, aes(displ, hwy)) +
  geom_point() +
  scale_y_continuous(breaks = seq(15, 40, by = 5)) # 15, starts value of sequence, 40 ending value of sequence, 5 as step size between values
```
...
Replacing a scale-colour scale
```{r}
...
```{r}
#customized points for shapes, important for black and white plots
ggplot(mpg, aes(displ, hwy)) +
...
```{r}
#set predefined colours using scale_colour_manual()
...
```{r}
#viridis colour scheme scale_colour_viridis()
...
Themes
```{r}
#Theme allows you to change some or all of the non-data elements of your plot with a theme
...
```
```{r}
#develop your own theme simply by setting some of the arguments to theme()
